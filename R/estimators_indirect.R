...
#'
#' @export
indirect_onestep <- function(data, trt, outcome, baseline, outcome_type = c("binomial"), folds = 5, trt_method = "default", learners_trt = c("mean", "glm"), learners_outcome = c("mean", "glm"), Qtilde = NULL, g = NULL, verbose = FALSE, control = standardization_control()) {
  if(length(outcome_type) > 1) outcome_type <- outcome_type[1]
  ...
  theta <- theta_indirect_onestep(task, trt_prop, ybar, Qtilde$predicted_outcomes, g, riesz)
  theta$g <- g
  theta$riesz <- riesz
  theta$Qtilde <- Qtilde
