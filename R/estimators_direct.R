#' Targeted Minimum Loss-Based Estimator of directly standardized mean outcomes
#'
...
#' Substitution Estimator of the directly standardized mean outcome
#'
#' @param data \[\code{data.frame}\]\cr
#' A \code{data.frame} containing all baseline, treatment, and outcome variables.
#' @param trt \[\code{character}\]\cr
...
#'
#' @export
direct_sub <- function(data, trt, outcome, baseline, outcome_type = "binomial", folds = 5, learners = c("mean", "glm"), control = smr_control()) {
  task <- tsmr_Task$new(
    data = data,
    trt = trt,
    outcome = outcome,
...
#' Probability Weighted Estimator of the directly standardized mean outcome
#'
#' @param data \[\code{data.frame}\]\cr
#' A \code{data.frame} containing all baseline, treatment, and outcome variables.
#' @param trt \[\code{character}\]\cr
...
#'
#' @export
direct_pw <- function(data, trt, outcome, baseline, trt_method = "default", outcome_type = "binomial", folds = 5, learners = c("mean", "glm"), control = smr_control()) {
  task <- tsmr_Task$new(
    data = data,
    trt = trt,
    outcome = outcome,
